-- ------------------------------------------------------------
-- 00055: fix the already-billed guard in
-- apply_standard_charges_to_existing (00053).
--
-- The guard decided "this horse has already been billed for X" by
-- comparing the standard charge's LABEL against misc_charges.category
-- only. Real hand-keyed charges do not follow that shape: on the live
-- Fire Cracker data the office's stall charge is DESCRIBED "Stall" but
-- filed under the broader CATEGORY "Stabling". A standard charge
-- labelled "Stall" therefore matched nothing and would have been added
-- on top of the one already there — a silent $185 double-charge to an
-- exhibitor, the exact failure the guard exists to prevent.
--
-- Caught by checking the live data before running the backfill, not by
-- the backfill running. Nothing was double-charged.
--
-- The guard now matches on category OR description, in both the
-- entry-attributed and the unattributed case. Everything else in the
-- function is unchanged from 00053.
-- ------------------------------------------------------------
...
begin
  ...
  loop
    ...
    loop
      ...
      --   2. an unattributed charge on the bill payer — a legacy row, or
      --      one the office keyed in by hand.
      --
      -- Both match on category OR description. Category alone was not
      -- enough: a hand-keyed charge is typically DESCRIBED "Stall" but
      -- filed under the broader CATEGORY "Stabling", so a standard
      -- charge labelled "Stall" slipped straight past the guard and
      -- would have been billed a second time.
      ...
      if exists (
        ...
        where mc.show_id = any(v_show_ids)
          ...
          and (mc.category = v_label or mc.description = v_label)
      ) or exists (
        select 1
        from public.misc_charges mc
        where mc.show_id = any(v_show_ids)
