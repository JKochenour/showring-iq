...
## Latest (2026-07-12, 9th session — "fix all the gaps"): after a competitive gap-analysis conversation, the user said "fix all the gaps" and 11 features were built in one continuous stretch, one commit each, all build/lint-clean: **refunds** (53937d6, migration 00034 — payments.is_refund/refund_of_payment_id, record_refund RPC), **trainer/barn billing** (53937d6, migration 00035 — entries.bill_to_trainer bills the trainer instead of owner/rider, billing.ts reworked), **per-run/conditional fees** (7b1fd9d, migration 00036 — per-run standard charges, late entry fee, close-out fee bulk-apply, card surcharge on record_payment), **multi-judge scoring** (79d8d1a, migration 00037 — new score_judges table, additive alongside scores, zero impact on 0/1-judge classes), **year-end/high-point standings** (d0b2132, migration 00038 — cross-show aggregation via class_affiliations.counts_for_year_end, which existed since 00018 but was unused until now), **1099/tax reporting** (dc8c63a, migration 00039 — people.tax_name only, NO SSN/EIN column, W-9 via existing documents/DocumentManager), **dual-show entry copy** (5799f43, no migration — "Copy from another show" on New Entry), **stall/camper/warm-up reservations** (c85989e, migration 00040 — one generic request→confirm→charge flow), **reports center + gate sheet** (1f282cd, no migration), **announcements** (2659a9b, no migration — broadcasts via existing Resend/Twilio). **IMPORTANT: this batch has NOT been live-verified** — migrations 00034 through 00040 need to be applied to Supabase in order, then every feature needs the same live-browser verification pass every earlier feature this session got. See `gap-closure-batch.md` in memory for the full writeup including known v1 simplifications (multi-judge is numeric-only, standings/payee-report are one-horse/one-rider only, close-out-fee's balance math is duplicated in raw SQL and could drift from billing.ts's JS version if that ever changes).
